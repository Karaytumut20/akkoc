...
const OrderSummary = () => {
  ...
  const handleQuantityChange = (productId, newQuantity) => {
    // Sepet miktarını güncellemek için AppContext'teki fonksiyonu kullan.
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // Sepet öğelerini Stripe'a göndermek için hazırla.
      // item.price alanı, AppContext'te hesaplanmış BİRİM FİYATTIR.
      ...
      if (url) {
        ...
        // Başarılı ödeme akışına yönlendirmeden hemen önce sepeti boşaltma mantığı (İsteğe bağlı, ancak genelde success_url'de yapılır)
        // setCartItems({}); 
      } else {
      ...
      <div className="space-y-5 mb-6 max-h-[60vh] md:max-h-[500px] overflow-y-auto">
        {Object.keys(cartItems).length === 0 ? (
          ...
          Object.values(cartItems).map((item, idx) => {
            ...
            // HATA KORUMASI: Fiyatı ve miktarı güvenli bir şekilde al.
            ...
            >
              ...
              <div className="flex-1 px-3 md:px-4">
                ...
                {/* HATA KORUMASI: price'ı toFixed ile güvenli kullan */}
                <p className="text-xs md:text-sm text-gray-500">{currency}{price.toFixed(2)}</p>
                {item.isBulk && (
                    <span className="text-xs font-medium text-green-600"> (Özel Fiyat)</span>
                )}
              </div>
              ...
              <div className="ml-2 md:ml-4 font-semibold text-gray-900 text-sm md:text-base">
                {/* HATA KORUMASI: itemTotal'ı toFixed ile güvenli kullan */}
