...
import Link from 'next/link'; // next/link'i import et
...
export default function TiffanyIconsGrid() {
  ...
  const fetchIcons = async () => {
    const { data, error } = await supabase
      ...
      .eq('brandicon', true); // 'brandicon' true olanları çekiyoruz

    if (error) {
      console.error('Icon ürünleri alınamadı:', error.message);
      setIcons([]);
    } else {
      // Supabase'den gelen veriyi işleyip state'e kaydediyoruz
      const productsWithImages = (data || []).map(item => ({
        id: item.id, // Ürün ID'sini alıyoruz
        title: item.name,
        image: item.image_urls && item.image_urls.length > 0
          ? Array.isArray(item.image_urls)
            ? item.image_urls[0] // Eğer dizi ise ilk elemanı al
            : JSON.parse(item.image_urls)[0] // Eğer string ise parse et ve ilk elemanı al
          : '/assets/shopnow.jpg', // fallback image
    ...
    <section className="w-full bg-white py-12 sm:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-xl sm:text-2xl font-serif text-gray-900 text-center mb-12 sm:mb-16">
          The Nestcome Icons {/* Başlık güncellenmiş */}
        </h2>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-8">
          {icons.map((item, index) => (
            <div key={item.id} className="text-center group">
              <div className="relative w-full aspect-square overflow-hidden bg-[#be531c]"> {/* Arka plan rengi güncellenmiş */}
                <Image
                  src={item.image}
                  alt={item.alt}
                  fill
                  ...
                  priority={index < 2} // İlk iki resmi öncelikli yükle
                />
              </div>

              <div className="mt-6">
                <h3 className="text-xl font-serif text-gray-900 mb-4">{item.title}</h3>
                {/* <a> etiketi yerine <Link> kullanıldı.
                  href attribute'ü `/product/${item.id}` olarak değiştirildi.
                  Bu sayede tıklandığında doğrudan ürün detay sayfasına gidilecek.
                */}
                <Link
                  href={`/product/${item.id}`} // Ürün ID'sine göre link oluşturuldu
