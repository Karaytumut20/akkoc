        ...
        <div className="flex gap-6 overflow-x-auto pl-4 pr-12 sm:grid sm:grid-cols-3 sm:gap-8 lg:gap-12 sm:overflow-visible scrollbar-hide snap-x snap-mandatory">
          {services.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="grid grid-rows-[auto_1fr] items-center justify-items-center text-center min-w-[80%] sm:min-w-0 flex-shrink-0 group transition-all duration-300 snap-start hover:scale-105 min-h-[340px]"
              >
                {/* Görsel */}
                <div
                  className={`relative mb-1 overflow-hidden flex items-center justify-center 
                    ${
                      isLast
