...
export default function MainNavbar() {
  ...
  return (
    <>
      ...
      <header className={`w-full pt-4 pb-2 px-5 sm:px-10 lg:px-16 ${headerClasses}`}>
        <div className="flex items-center justify-between relative">
          ...
          <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 cursor-pointer" onClick={() => router.push("/")}>
            <Image className="w-28 md:w-32" src={logoSrc} alt="logo" width={128} height={64} />
          </div>
            ...
            {user ? (
              <div className="relative" ref={userMenuRef}>
                <button onClick={() => setIsUserMenuOpen((v) => !v)} className="flex items-center gap-2 p-2 rounded-full hover:bg-black/10 transition">
                  <Image className="w-5 h-5" src={assets.user_icon} alt="user icon" width={20} height={20} />
          ...
          <div ref={searchRef} className="relative mt-4 max-w-md mx-auto">
            <form onSubmit={handleSearchSubmit} className="flex">
              <input
                ...
                autoFocus
                className="w-full px-4 py-2 rounded-md focus:outline-none focus:ring-2 bg-gray-100 text-gray-800 placeholder-gray-500 focus:ring-orange-500"
              />
            </form>
        ...
        <nav className="mt-6 hidden lg:flex justify-center space-x-10 text-sm font-light tracking-[0.25em] uppercase text-gray-700">
          {navLinks.map((item) => (
